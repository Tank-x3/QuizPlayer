document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadAllQuizzes() {
        try {
            // クイズリストとカテゴリ設定を並行して取得
            const [listResponse, catResponse] = await Promise.allSettled([
                fetch('data/quiz_list.json'),
                fetch('data/categories.json')
            ]);

            // カテゴリ設定の読み込み（失敗しても続行）
            if (catResponse.status === 'fulfilled' && catResponse.value.ok) {
                try {
                    categoriesOrder = await catResponse.value.json();
                } catch (e) {
                    console.warn('categories.json read failed', e);
                }
            }

            if (listResponse.status === 'rejected' || !listResponse.value.ok) {
                throw new Error('設定ファイル "quiz_list.json" が見つかりません。');
            }
            const fileList = await listResponse.value.json();
    ...
    function displayQuizList() {
        ...
        // カテゴリのソート
        const sortedCategories = Object.keys(quizzesByCategory).sort((a, b) => {
            const indexA = categoriesOrder.indexOf(a);
            const indexB = categoriesOrder.indexOf(b);

            // 両方とも設定リストにある場合、その順序でソート
            if (indexA !== -1 && indexB !== -1) return indexA - indexB;
            // 片方だけある場合、ある方を優先（上に来る）
        ...
        sortedCategories.forEach(category => {
            ...
            details.className = 'quiz-category-details';

            const summary = document.createElement('summary');
            summary.innerHTML = `<h3>${category}</h3>`;
            ...
            quizzesByCategory[category].forEach(quiz => {
                ...
                contentDiv.appendChild(quizCard);
            });

            details.appendChild(contentDiv);
            quizListContainer.appendChild(details);
        });
    }
